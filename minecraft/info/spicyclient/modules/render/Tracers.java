		...
		if (e instanceof EventRender3D && e.isPre()) {
			...
			for (Entity entity : mc.theWorld.getLoadedEntityList()) {
				...
				if (entity instanceof EntityPlayer && ((EntityPlayer)entity) != mc.thePlayer) {
					...
					try {
						if (entity instanceof EntityPlayer && SpicyClient.config.antibot.isEnabled() && !mc.isSingleplayer() && mc.getCurrentServerData().serverIP.toLowerCase().contains("hypixel") && mc.getNetHandler().getPlayerInfo(((EntityPlayer)entity).getUniqueID()).responseTime > 1) {
